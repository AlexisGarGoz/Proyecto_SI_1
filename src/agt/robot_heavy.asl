...
+!test_movement : true <-
    ...
    .print("Movimiento 1: Aproximación cuidadosa al área de entrada (2,2)");
    move_to(2, 2);
    .wait(3000);
    
    .print("Movimiento 2: Navegar a zona central (8,6)");
    move_to(8, 6);
    .wait(3000);
    
    .print("Movimiento 3: Ir a zona de estanterías GRANDES (12,11)");
    move_to(12, 11);
    .wait(3000);
    
    .print("Movimiento 4: Explorar más estanterías grandes (14,10)");
    move_to(14, 10);
    .wait(3000);
    
    .print("Movimiento 5: Retornar a zona segura (6,6)");
    move_to(6, 6);
    .wait(3000);
...
+task(CId, ShelfId) : not state(idle) <-
    .print("[HEAVY] Ocupado con carga pesada, rechazando: ", CId).
...
+!navigate_to_shelf(ShelfId) : true <-
    .print("[HEAVY] Localizando estantería: ", ShelfId);
    get_shelf_info(ShelfId); 
    .wait(shelf_pos(ShelfId, X, Y), 2000); 
    ?shelf_pos(ShelfId, X, Y);
    .print("[HEAVY] Yendo a la posición real: ", X, ",", Y);
    move_to(X, Y);
    .wait(1000).
...
+error(container_too_heavy, Data) : carrying(CId) <-
    ...
    .print("Este es el robot más fuerte, contenedor imposible de transportar");
    -+state(idle);
    -+carrying(none);
    -task(CId, _).
...
+error(ErrorType, Data) : true <-
    .print("[HEAVY] Error detectado: ", ErrorType, " - ", Data);
    -+state(idle);
    -+carrying(none).
